$(document).ready(function () {
    ...
    setInterval(function () {
        $(".bg-image").css({
            "background-image": "-o-linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url(img/header/" + images[i] + ")",
            "background-image": "linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url(img/header/" + images[i] + ")",
            "background-image": "-webkit-gradient(linear, left top, left bottom, from(rgba(0, 0, 0, 0.7)), to(rgba(0, 0, 0, 0.7))), url(img/header/" + images[i] + ")"
        });
    ...
    $('#accordion-option').on('show.bs.collapse', function () {
        $('.icon-expand').removeClass('fa-caret-square-down');
        $('.icon-expand').addClass('fa-caret-square-up');
    });

    $('#accordion-option').on('hide.bs.collapse', function () {
    ...
    $('form[name="request-form"]').submit(function (e) {
        e.preventDefault();
        sendRequest();
